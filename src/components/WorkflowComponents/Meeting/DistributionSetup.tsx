...
import { SAMPLE_TEMPLATE_DATA } from "./emailTemplates";
import { TemplateSelector } from "./TemplateSelector";
import type { DistributionSettings, TemplateId, UpdateDistributionSettingsRequest, CreateDistributionSettingsRequest } from "./distributionTypes";
import { 
...
export const DistributionSetup: React.FC<DistributionSetupProps> = ({
	minutesNode,
	distributeNode,
	meetingName,
}) => {
	...
	// Parse existing distribution list from node
	useEffect(() => {
		if (distributeNode?.output) {
			const emails = parseEmailList(distributeNode);
			const parsedRecipients = emails.map(email => ({
				email,
				selected: true,
			}));
			...
			// No distribute node, still mark as initialized
			setOriginalSettings({
				template: "notion",
				subject: emailSubject,
				recipients: [],
	...
	const parseEmailList = (node: NodeData): string[] => {
		let emails: string[] = [];
		
		if (Array.isArray(node.output)) {
			emails = node.output.filter((item: unknown) => 
				typeof item === "string" && item.includes("@"),
			);
		} else if (typeof node.output === "string") {
			try {
		...
		} else if (node.output?.email_list && Array.isArray(node.output.email_list)) {
			emails = node.output.email_list.filter((item: unknown) => 
				typeof item === "string" && item.includes("@"),
			);
		} else if (node.output?.emails && Array.isArray(node.output.emails)) {
			emails = node.output.emails;
		}
		...
		return (
			<div className="h-full flex items-center justify-center">
				<div className="text-center py-12">
					...
					<h3 className="text-lg font-medium text-gray-900 mb-2">No Minutes Available</h3>
					<p className="text-sm text-gray-500 max-w-sm">
						Meeting minutes will appear here once they&apos;re generated after the meeting.
					</p>
				</div>
			</div>
		);
