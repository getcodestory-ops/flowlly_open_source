...
import { MessageSquarePlus, Eye, FolderOutput } from "lucide-react";
												...
												>
													<div className="flex items-center gap-2 min-w-0 flex-1">
														...
														<Button
															...
															variant="ghost"
														>
															<MessageSquarePlus className="mr-1" size={14} />
															<span className="text-xs">Add to Chat</span>
														</Button>
														<Button
															className={clsx(
																"flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-50 hover:bg-gray-100 text-gray-700 border border-gray-200",
																contextFolder.id === folder.id && "bg-blue-50 text-blue-700 hover:bg-blue-100",
															)}
															onClick={(e) => {
																e.stopPropagation();
																setContextFolder(folder.id, folder.name);
																toast({
																	title: "Output folder set",
																	description: `All new files generated by chat will be saved in ${folder.name}`,
																	duration: 3000,
																});
															}}
															size="sm"
															title={contextFolder.id === folder.id ? "Current chat output folder" : "Set as chat output folder"}
															variant="ghost"
														>
															<FolderOutput className="mr-1" size={14} />
															<span className="text-xs">
																{contextFolder.id === folder.id ? "Output Folder" : "Set Output"}
															</span>
														</Button>
													</div>
												</div>
											...
											return (
												...
												>
													<div className="flex items-center gap-2 min-w-0 flex-1">
														...
														<Button
															className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-50 hover:bg-gray-100 text-gray-700 border border-gray-200 mr-2"
															onClick={(e) => {
																e.stopPropagation();
																toggleItemSelection({
																	id: file.id,
