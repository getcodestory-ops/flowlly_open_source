...
export { useViewStore } from "@/utils/viewStore";
...
export const useStore = create<State>()((set, get) => ({
	...
	setActiveProject: (activeProject: ProjectEntity | null) => {
		...
		// Sync document store immediately so getScopedFolderKey uses correct project (avoids flash of wrong/empty folder)
		const docStore = useDocumentStore.getState();
		docStore.setProjectContext(
			activeProject?.project_id ?? "",
			docStore.isProjectWide,
		);
		set(() => ({ activeProject }));
	},
